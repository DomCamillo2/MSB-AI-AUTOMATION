# Security Policy

## Supported surfaces

This repository contains the public marketing website for MSB AI & Automation,
including the static site export and the IONOS PHP contact endpoint.

## Reporting a vulnerability

Please report security issues privately by e-mail to:

**kontakt@msb-ai.de**

Include:

- a short description of the issue
- the affected URL or file, if known
- steps to reproduce, if practical

Do not open a public GitHub issue for security reports.

## Response

We aim to acknowledge reports within a few business days and to remediate
confirmed issues on a priority basis. Please give us a reasonable window to
investigate before any public disclosure.
